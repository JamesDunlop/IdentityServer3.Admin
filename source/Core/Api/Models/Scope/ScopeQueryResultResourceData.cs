...
namespace IdentityAdmin.Api.Models.Scope
{
    public class ScopeQueryResultResourceData : QueryResult<ScopeSummary>
    {
        ...
        public ScopeQueryResultResourceData(QueryResult<ScopeSummary> result, UrlHelper url, ScopeMetaData meta)
        {
            ...
            {
                var links = new Dictionary<string, string> { {"detail", url.LinkWithoutProtocol(Constants.RouteNames.GetScope, new { subject = scope.Data.Subject })}
                };
                if (meta.SupportsDelete)
                {
                    links.Add("delete", url.LinkWithoutProtocol(Constants.RouteNames.DeleteScope, new { subject = scope.Data.Subject }));
                }
                scope.Links = links;
            }
        }
