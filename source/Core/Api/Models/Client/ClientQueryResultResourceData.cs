        ...
        public ClientQueryResultResourceData(QueryResult<ClientSummary> result, UrlHelper url, ClientMetaData meta)
        {
            ...
            foreach (var client in this.Items)
            {
                var links = new Dictionary<string, string> {
                    {"detail", url.LinkWithoutProtocol(Constants.RouteNames.GetClient, new { subject = client.Data.Subject })}
                };
                if (meta.SupportsDelete)
                {
                    links.Add("delete", url.LinkWithoutProtocol(Constants.RouteNames.DeleteClient, new { subject = client.Data.Subject }));
                }
                client.Links = links;
            }
        }
